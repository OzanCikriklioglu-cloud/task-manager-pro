...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['complete_task'])) {
    ...
    $updateStmt = $pdo->prepare("UPDATE tasks SET status = 'done', completed_at = NOW() 
                                WHERE id = :id AND user_id = :user_id");
    $updateStmt->execute([':id' => $task_id, ':user_id' => $user_id]);
    
    // Redirect back to the same page with current filters
    header("Location: taskmanager.php?filter=$filter&sort=$sort&order=$order&category=$categoryFilter");
    exit;
}
...
// Status filter
if ($filter === 'completed') $sql .= " AND t.status='done'";
elseif ($filter === 'incomplete') $sql .= " AND t.status != 'done'";
// Category filter
...
$now = time();
foreach ($tasks as &$t) {
    if ($t['status'] !== 'done' && $t['due_date'] && strtotime($t['due_date']) < $now) {
        $t['status'] = 'failed';
    }
}
unset($t);
...
<html lang="tr">
  <head>
    ...
    <style>
      ...
      .btn-complete{background:#28a745;color:#fff;padding:4px 8px;border:none;border-radius:4px;cursor:pointer}
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              ...
              <!-- Bitir butonu - sadece tamamlanmamış görevler için -->
              <?php if($t['status'] !== 'done'): ?>
                <form method="POST" style="display:inline;margin-left:5px;">
                  <input type="hidden" name="task_id" value="<?=$t['id']?>">
                  <button type="submit" name="complete_task" class="btn-complete">Bitir</button>
                </form>
              <?php endif; ?>
            </td>
          </tr>
